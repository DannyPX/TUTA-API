...
// Send departure times depended on station
app.get('/api/ns/departures/:code', (req, res) => {
  var params =  `maxJourneys=25&` + 
                `lang=nl` + 
                `&station=${req.params.code}`
...
// Send Trip information depended on start and end station.
app.get('/api/ns/getTrips/:origin&:destination', (req, res) => {
  var params =  `travelClass=SECOND_CLASS&` + 
                `originTransit=false&` +
                `originWalk=false&` + 
                `originBike=false&` + 
                `originCar=false&` + 
                ...
                `passing=false&` + 
                `fromStation=${req.params.origin}&` + 
                `toStation=${req.params.destination}`
  const Http = new XMLHttpRequest()
...
function NSHTTPRequest(Http, url, params) {
  ...
  Http.setRequestHeader("Ocp-Apim-Subscription-Key", process.env.NSAPI_KEY)
  Http.send()
}

// Send all possible locations based on given keyword
app.get('/api/9292/searchLocation/:keyword', (req, res) => {
  var params =  `lang=nl-NL` + 
...
// Send Trip information depended on start, end station and departure time.
...
// Give weather forecast based on lat. and long.
